...
def doer(next_line):
    with line_bind(next_line(), int) as (one,):
        y = one.to_bytes(4, byteorder='little', signed = one < 0)
        another = int.from_bytes(y, byteorder='big', signed=True)
        print("{} converts to {}".format(one, another))
